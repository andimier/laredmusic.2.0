<?php
	...
	if(isset($_POST['botonsubirimagen']) ){
		...
		if($_FILES['nombre_archivo']['error'] == 1){
			...
			if($archivoparaborrar == 'photo.png'){
			...
			if(move_uploaded_file($temp_path, $destino)){
				...
				if(mysql_affected_rows()==1){
					...
					//if(isset($texto_id)){
					//	header('Location: editar-textos.php?sub-contenido='.$subcontenido.'&texto_id=' . $texto_id . '&contenido=' . $contenido);
					//}else{
					//	header('Location: editar-contenido.php?contenido_id=' . $id);
					//}
					
					header('Location: editar-noticia.php?noticia_id=' . $id);
